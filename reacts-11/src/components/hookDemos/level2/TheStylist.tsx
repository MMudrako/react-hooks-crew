...
/* you've seen it many times in movies before a TV show starts,
 a stylist does last-minute make-up touch up for the host -
 that's the job of the useLayoutEffect right before DOM is painted 
 it measures and adjusts layout(obviously)) or even state. The main difference 
 between useEffect and useLayoutEffect is that first async the latter is sync, 
 so if we want to make changes to the code right before user sees the new render,
...
export default function TheStylist() {
    ...
    const saveAgent = () => {
        ...
        if (storedAgent === JSON.stringify(defaultAgent)) {
            console.log('Stored agent is default:', agent);
        } else {
            setAgent(prevAgent => ({
                ...prevAgent,
